...
class SimuRAWorldModel:
    ...
    async def simulate_action(
        ...
        logger.info(
            ...
            extra={
                ...
                "avg_success_prob": (
                    sum(s.success_probability for s in simulations) / len(simulations)
                    if simulations
                    else 0
                ),
            },
    ...
    async def _simulate_single_action(
        ...
        """
        if self._gemini:
            prediction = await self._llm_predict_outcome(current_state, action, context)
        else:
            prediction = predict_outcome_heuristic(current_state, action)
    ...
    async def _llm_predict_outcome(
        ...
        try:
            prediction: SimulationPrediction = await self._gemini.simulate_action(
                state=current_state, action=action, context=context
            )
            ...
            return predict_outcome_heuristic(current_state, action)
    ...
    def select_best_action(
        ...
        if strategy == "max_success":
            ...
            best = max(
                viable,
                key=lambda s: (
                    s.success_probability * 0.6 - s.risk_score * 0.3 + s.confidence * 0.1
                ),
            )
        elif strategy == "conservative":
            best = min(viable, key=lambda s: (s.risk_score, -s.success_probability))
        else:
    ...
    def record_outcome(
        ...
        new_accuracy = self._dyna_state.prediction_accuracy * (1 - lr) + (
            1.0 if predicted_correct else 0.0
        ) * lr
